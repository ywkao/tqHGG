...
void TMVAClassificationApplication( TString myMethodList = "" )
{
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      histBdt->Fill( reader->EvaluateMVA( "BDT method" ) , tree_event_weight);
      if(pass_signal_region)
      {
        histBdt_signalRegion->Fill( reader->EvaluateMVA( "BDT method" ) , tree_event_weight);
      }
