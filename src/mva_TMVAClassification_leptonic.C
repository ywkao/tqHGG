...
int TMVAClassification( TString myMethodList = "" ){
   ...
   // Here the preparation phase begins(example){{{
   ...
   // std::cout << "--- TMVAClassification       : Using input file: " << input->GetName() << std::endl;
    ...
    //}}}
    // Open files{{{
    TFile *input[NUM_training_samples];
    for(int i=0; i<NUM_training_samples; ++i){
        input[i] = TFile::Open( fname[i] );
        std::cout << "--- TMVAClassification       : Using input file: " << input[i]->GetName() << std::endl;
    }
    //}}}
   ...
   // TTree *background     = (TTree*)input->Get("TreeB");
    //0: sig; ohters: bkg
    TTree *MC_samples[NUM_training_samples];
    for(int i=0; i<NUM_training_samples; ++i)
        MC_samples[i] = (TTree*)input[i]->Get("myAnalysisTree");
    //}}}
    // Output & factory{{{
   ...
   if (Use["BDTG"]) // Gradient Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG",
                           "!H:!V:NTrees=1000:MinNodeSize=2.5%:CreateMVAPdfs:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2" );
