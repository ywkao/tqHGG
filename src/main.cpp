...
int main(int argc, char *argv[]){
    ...
    for(int ientry=0; ientry<nentries; ientry++){
        ...
        //--------------------
        hist_DiPhoInfo_leadIDMVA_ori -> Fill(DiPhoInfo_leadIDMVA);
        hist_DiPhoInfo_subleadIDMVA_ori -> Fill(DiPhoInfo_subleadIDMVA);
        hist_inv_mass_diphoton_ori -> Fill(DiPhoInfo_mass);
        //--------------------
        ...
        if(!CUT_num_nonbjets_less_than_2 && !CUT_no_bjet_events){// Requirement for at least 1 top!
            for(int i=0; i<vec_nonbtagged_jets.size(); i++){
                for(int j=i+1; j<vec_nonbtagged_jets.size(); j++){
                    ...
                    for(int k=0; k<vec_btagged_jets.size(); k++){
                        ...
                        chi2 = (dijet_invariant_mass-w_boson_mass)*(dijet_invariant_mass-w_boson_mass) + (trijet_invariant_mass-top_quark_mass)*(trijet_invariant_mass-top_quark_mass);
        ...
        best_dijet = CUT_no_any_top_candidate ? best_dijet : vec_nonbtagged_jets[wjetindices[0]] + vec_nonbtagged_jets[wjetindices[1]];
        dijet_invariant_mass = CUT_no_any_top_candidate ? -999. : best_dijet.M();
        //------------------------------
        double dijet_jet1_pt = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[0]].Pt();
        double dijet_jet1_eta = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[0]].Eta();
        double dijet_jet1_phi = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[0]].Phi();
        double dijet_jet2_pt = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[1]].Pt();
        double dijet_jet2_eta = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[1]].Eta();
        double dijet_jet2_phi = CUT_no_any_top_candidate ? -999. : vec_nonbtagged_jets[wjetindices[1]].Phi();
        //------------------------------
        double dijet_eta_difference = CUT_no_any_top_candidate ? -999. : fabs(dijet_jet1_eta - dijet_jet2_eta);
        double dijet_phi_difference = CUT_no_any_top_candidate ? -999. : fabs(dijet_jet1_phi - dijet_jet2_phi);
