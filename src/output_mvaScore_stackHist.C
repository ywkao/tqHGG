...
bool bool_isHut = false;
bool bool_isHct = !bool_isHut;
...
void MakeStackHist(const char* histName){
    ...
    if(PrintTexStyle && isCoarser){
        ...
        CalculateHistYields_mva_score("GJet\t\t", hist_tqh_GJet);
        CalculateHistYields_mva_score("TGJets\t\t", hist_tqh_TGJets);
        CalculateHistYields_mva_score("TTGJets\t\t", hist_tqh_TTGJets);
        CalculateHistYields_mva_score("TTGG\t\t", hist_tqh_TTGG);
        CalculateHistYields_mva_score("TTJets\t\t", hist_tqh_TTJets );
        CalculateHistYields_mva_score("DYJetsToLL\t", hist_tqh_DYJetsToLL );
        CalculateHistYields_mva_score("VG\t\t", hist_tqh_VG );
        CalculateHistYields_mva_score("VV\t\t", hist_tqh_VV );
        CalculateHistYields_mva_score("Higgs\t\t", hist_tqh_Higgs);
        CalculateHistYields_mva_score("ggH\t\t", hist_tqh_ggH);
        CalculateHistYields_mva_score("VBF\t\t", hist_tqh_VBF);
        CalculateHistYields_mva_score("VH\t\t", hist_tqh_VH);
        CalculateHistYields_mva_score("ttH\t\t", hist_tqh_ttH);
        CalculateHistYields_mva_score("QCD\t\t", hist_tqh_QCD);
        CalculateHistYields_mva_score("sig_tt_hut\t", hist_sig_tt_hut);
        CalculateHistYields_mva_score("sig_st_hut\t", hist_sig_st_hut);
        CalculateHistYields_mva_score("sig_tt_hct\t", hist_sig_tt_hct);
        CalculateHistYields_mva_score("sig_st_hct\t", hist_sig_st_hct);
        printf("\\hline\n");
        CalculateHistYields_mva_score("resonant background\t", hist_tqh_resbkg[NUM_resbkg]);
        CalculateHistYields_mva_score("non-res background\t", hist_tqh_nonresbkg[NUM_nonresbkg]);
    ...
    double total_entries_data = 100.;
    double scale_sig_tt_hut = 100.;
    double scale_sig_st_hut = 100.;
    double scale_sig_tt_hct = 100.;
    double scale_sig_st_hct = 100.;
...
//
    ...
    //--- normalize to total entries of data ---//
    total_entries_data = 0.01;
    scale_sig_tt_hut = 0.01;
    scale_sig_st_hut = 0.01;
    scale_sig_tt_hct = 0.01;
    scale_sig_st_hct = 0.01;
    if(normalizeSigEntryToData){
        total_entries_data = hist_tqh_data[NUM_data]->Integral();
        scale_sig_tt_hut = total_entries_data / hist_sig_tt_hut->Integral() ; hist_sig_tt_hut->Scale(scale_sig_tt_hut);
        scale_sig_st_hut = total_entries_data / hist_sig_st_hut->Integral() ; hist_sig_st_hut->Scale(scale_sig_st_hut);
    ...
    if(!isCoarser){
        ...
        double _max_scope = (_max_b > _max_s) ? _max_b : _max_s;

        hist_yield_b->SetStats(0);
        hist_yield_b->SetMaximum(_max_scope*scale);
        hist_yield_b->Draw();
        hist_yield_s->Draw("same");
        TLegend *legend_yield = new TLegend(0.60,0.70,0.90,0.90);
        legend_yield->SetLineColor(0);
...
string GetXtitleAccordingToHistName(const char* histName){
    if((string)histName == "MVA_BDT") return "BDT output score";
    //----------------------------------------------------------------------
...
string GetYtitleAccordingToHistName(const char* histName, double BinWidth){
    ...
    //----------------------------------------------------------------------
    if((string)histName == "MVA_BDT") return str_ytitle_1;
    //----------------------------------------------------------------------
...
void GetSignificanceHist(bool isZA, TH1D *& hist_significance, TH1D *& hist_yield_s, TH1D *& hist_yield_b, TH1D* sig, TH1D* bkg){
    ...
    for(int i=0; i<nbins; ++i){
        ...
        for(int j=i; j<nbins; ++j){
            s += sig->GetBinContent(j+1);
            b += bkg->GetBinContent(j+1);
            unc_s += pow(sig->GetBinError(j+1), 2);
            unc_b += pow(bkg->GetBinError(j+1), 2);
        }
        unc_s = sqrt(unc_s);
        unc_b = sqrt(unc_b);
        hist_yield_s->SetBinContent(i+1, s*100.);
        hist_yield_s->SetBinError(i+1, unc_s*100.);
    ...
    hist_yield_s->SetMarkerColor(kRed);
    hist_yield_s->SetMarkerStyle(20);
    hist_yield_s->SetMarkerSize(1.2);
    hist_yield_b->SetMarkerColor(kBlue);
    hist_yield_b->SetMarkerStyle(20);
    hist_yield_b->SetMarkerSize(1.2);
    hist_significance->SetFillColor(kBlack);
    hist_significance->SetLineColor(kBlack);
    hist_significance->SetLineWidth(2);
    hist_significance->SetMarkerStyle(20);
    hist_significance->SetMarkerSize(1.2);
    hist_significance->SetTitle("");
...
double calculate_ZA(float s, float b){
    ...
    double q1 = 2. * ( ((s+b) * log(1. + (s/b) )) - s);
    if(q1 <= 0) return 0;
    else return sqrt(q1);
}

double unc_ZA(float s, float b, float ds, float db){
    double criterion_lowerBound = 1e-3;
    double ZA = calculate_ZA(s,b);
    double Zs = log(1. + (s/b)) / ZA;
    double Zb = (log(1. + (s/b)) - (s/b)) / ZA;
    double error = (ZA < criterion_lowerBound) ? 0. : sqrt((Zs*ds)*(Zs*ds) + (Zb*db)*(Zb*db));
    return error;
}
